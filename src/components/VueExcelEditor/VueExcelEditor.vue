...
export default defineComponent({
  ...
  data () {
    return {
      pageSize: this.noPaging ? 999999 : 20,
      version: '1.3',
  ...
  computed: {
    ...
    pageBottom () {
      ...
      return this.pageTop + this.pageSize > this.table.length ? this.table.length : this.pageTop + this.pageSize
    },
    setting: {
      get () {
        return null
  ...
  methods: {
          ...
          case 40: // Down Arrow
            ...
            if (this.autocompleteInputs.length === 0) {
              ...
              if (this.autocompleteSelect >= 10) {
                ...
                const { scrollTop } = this.$refs.autocomplete
    ...
    sort (n, pos) {
      ...
      setTimeout(() => {
        ...
        if (sorting === null) {
          ...
          } else { sorting = (a, b) => String(a[name]).localeCompare(String(b[name])) }
        }
        this.value.sort((a, b) => sorting(a, b) * -n)
        this.sortPos = colPos
        this.sortDir = n
        this.$forceUpdate()
        this.processing = false
          ...
          try {
            ...
            while (pass < 2) {
              ...
              await Promise.all(importData.map(async (line, i) => {
                ...
                if (keys.length) {
                  // locate match record
                  rowPos = this.table.findIndex((v) => keys.filter((f) => typeof v[f.name] !== 'undefined' &&
    ...
    */
    ...
    selectRecord (rowPos, eventName = 'select') {
    ...
    unSelectRecord (rowPos, eventName = 'unSelect') {
      if (typeof this.selected[rowPos] !== 'undefined') {
    ...
    moveInputSquare (rowPos, colPos) {
      ...
      this.$emit('cell-focus', {
        rowPos, colPos, cell, record: this.currentRecord
      })
    ...
    newRecord (rec, selectAfterDone, noLastPage, isUndo) {
      ...
      this.fields.map((f) => {
        let localRec = rec[f.name]
        if (typeof localRec === 'undefined') {
          if (f.keyField) { localRec = `§${this.tempKey()}` } else { localRec = null }
        }
        return localRec
      })
    ...
    hashCode (s) {
      // eslint-disable-next-line no-return-assign
      return s.split('').reduce((a, b) => a = ((a << 5) - a) + b.charCodeAt(0) | 0, 0)
    },
    lazy (p, delay, p1) {
